...
bool Circle::draw_midpoint(Canvas* canvas, int cx, int cy, int r, Color color)
{
    r--;
    if (r < 0 || cx + r < 0 || cx - r >= canvas->width || cy + r < 0 || cy - r >= canvas->height) return false;
...
bool Circle::draw_filled(Canvas* canvas, int cx, int cy, int r, Color color)
{
    r--;
    if (r < 0 || cx + r < 0 || cx - r >= canvas->width || cy + r < 0 || cy - r >= canvas->height) return false;
...
// bool Circle::getCircle(float x1, float y1, float x2, float y2, float x3, float y3, float* cx, float* cy, float* r)
// {
